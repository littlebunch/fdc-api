...
curl -u Administrator:password -XPOST -d 'statement=CREATE INDEX `idx_fd` ON `gnutdata`(foodDescription) USING GSI WITH {"defer_build":true}' http://localhost:8093/query/service
curl -u Administrator:password -XPOST -d 'statement=CREATE INDEX `idx_company` ON `gnutdata`(company) USING GSI WITH {"defer_build":true}' http://localhost:8093/query/service
